...
(cd /tmp/test-repo$$ >/dev/null; ${REPOTOOL:-repotool} export) | ./hg-to-fi | sed -e 1d -e '/^#legacy-id/d' | diff --text -u --label testrepo2.fi testrepo2.fi --label repotool-export - >/tmp/out$$
