...
branchify.svn::
   No standard structure, liftcheck will try to use --nobranch which
   fails. Used by branchify.tst.

branchmap.svn::
   No standard structure, liftcheck will try to use --nobranch which
   fails. Used by branchmap.tst.

branchreplace.svn::
   Conversion is correct - messages due to trimmed branches.

emptybranch.svn::
   A branch is made outside the normal structure, which is not detected
   by repotool branches. That branch is empty and yet not tagified.

expunge-copyfrom.svn::
   No standard structure, liftcheck will try to use --nobranch which
   fails. Used by expunge-copyfrom.tst.
...
swap.svn::
   No standard structure, liftcheck will try to use --nobranch which
   fails. Used in a repocutter test.
...
Passing but with some harmless messages:

symlink.svn::
   diff tries to follow the symlink inside the repository, which fails
