...
emptybranch.svn::
   BUG: The empty branch should be tagified but is not.
   Look for a way to fix this that doesn't break rhe
   myers1 case.
...
gitignore.svn::
   ...
   .gitigmore, as it should. Translation is correct.  Used by 
   userignores.tst

myers1.svn::
   Content is correct.  One message due to empty branch.
...
samplebranch.svn::
   BUG: Git translation preserves a deleted branch. Used in
...
vanilla.svn::
   Translation is correct.  Has an emptycommit, and there's a message
   due to a property setting. Used in repocutter and repotool tests.
