...
To allow the code to emit a stack trace, set anny log bit.
...
tagpollute.svn::
   BUG: Tag root attrbuted to wromng place due to partial copy.
   Content mismatch, isiolating one defect. This is derived from
   references.svn, isolating one defect.
...
The 'top' view gives you the same data in tabular form:
  ...
  257.19s  8.98%   8.98%  257.19s   8.98%  main.orderedIntSet.Contains
  229.12s  8.00%  16.99%  258.46s   9.03%  runtime.findObject
  144.93s  5.06%  22.05%  215.61s   7.53%  syscall.Syscall
  114.48s  4.00%  26.05%  433.07s  15.13%  runtime.mapassign_faststr
   99.77s  3.48%  29.53%   99.77s   3.48%  memeqbody

This is telling us that (a) disk I/O (syscall.Syscall) is slow, (b)
garbage collection is a little worse (runtime.findObject), (c)
membership tests on orderedIntSets are taking quite a lot of time
(main.orderedIntSet.Contains), and (d) that assigning things to maps
is actually worse (runtiem.mapassign_faststr).
...
if we could query our orderedIntSets less frequently, that'd be a good
...
but I haven't yet explored this feature.
