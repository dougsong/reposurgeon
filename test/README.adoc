...
node-kind-first.svn::
   Translation is corrext, but one of the property settings triggers
   a spurious message. Not a real bug.  Not used by any test.

nontipcopy.svn::
   Translation cottect, but has emptycommits.

permcopy2.svn::
   Translation is correct. Hascan emptycommit. An empy spource branch
