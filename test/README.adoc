...
You can use singlelift -d to dump a fast import stream made
from a named Subversion dump.
...
$ goreposurgeon "read ." "write >rs.fi"

The ability to dump profile data is built into reposurgeon itself:

$ goreposurgeon "verbose 1" "profile all reposurgeon" "read <rs.fi"
