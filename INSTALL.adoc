...
---------------------------------------------------------------------------
sudo apt-get install golang-1.12 diffutils            # For running
sudo apt-get install asciidoctor                      # To render docs
sudo apt-get install golint shellcheck pylint         # For testing
sudo apt-get install subversion mercurial rsync       # For testing
...
diff is required for repotool to do repository.
...
golint, shellcheck, pylint, subversion, mercurial, and rsync are
required for the regression tests, but not for running the code.  Very
old versions of Subversion (certainly 1.6 or earlier, but not 1.9 or
