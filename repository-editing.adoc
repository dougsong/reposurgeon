...
Multi-line contents with no summary::
    ...
    milting comments should be edited into this form.
...
supposing there were, moving content and metadata from git's DAG
...
reposurgeon test suite, describing a history with a two commit to a
single file:
...
history because they would clash with those generated from
...
`svn:global-ignores` propertiesxet, e.g. in a site configuration file)
...
(c) If you were using the standard-workflow Makefile generated by
