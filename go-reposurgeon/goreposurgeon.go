...
// Rhuis one is special because it's used directly in the Subversion
// dump parser, as well as in the VCS capability table.
const subversionDefaultIgnores = `
							...
							ancestor, _ = found.(*NodeAction) 
                                        ...
                                        if node.blob != nil {
                                                if sp.hashmap[node.contentHash] != nil  {
                                                        ...
                                                        // content hash.  Don't record                                                        // them, otherwise they'll all
                                                        // collide :-)
                                                        if node.contentHash != "" {
        ...
        for i, event := range sp.repo {
                if commit, ok := event.(*Commit); ok {
                        ...
                        if event.Comment == nil {
                                sp.gripe(fmt.Sprintf("r%s has no comment", event.legacy)ID)
                                continue
                        }
