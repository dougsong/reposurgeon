...
This is an afterr-action report on a Python-to-Go translation of a
...
The GCC history at over 280K commits is the straw that broke the
...
expect up to a 40x speedup, which was in line with published Python
...
22% in I wrote https://gitlab.com/esr/pytogo[a fast, crude, incomplete
Python-to-Go source translator] to assist the process.

The man barrier to translation was that, while at 14KLOC of Python
reposurgeon was not especially large, it was very *dense*.  It's a DSL
that's a structure editor for attributed DAGs - algorithmically
...
The a;gorithmic density of reposurgeon is such that it would be a
...
Yo make this properly parallel to the way iteration via range works,
we probably want a second form with a bool passed back:
...
I suggest that this be adopted for a future release of Go. Small, easy
...
2. Iyterators are felt by their absence and would be easy to add.

3. A technical writer with an outside-in view of the language should
