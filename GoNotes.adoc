...
expecially large, it is very *dense*.  It's a DSL that's a tructure
editor for attributed DAGs - algorithmically complex, bristling with
graph theory, FSMs, parsing, tricky data structures, two robot
harnesses driving other tools, and three different
...
I suggest that this be adopted for Go 2. Small, easy new evaluation rule,
...
I'm trying very hard to avoid that. It's that Go's minimalistic approch
actually...works.
