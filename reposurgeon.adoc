...
+verbose+ [ _n_ ]::
   'verbose 1' (the deault) enables error and warning messages,
   'verbose 0' disables them. Higher levels of verbosity are available
   but intended for developers only.
...
+relax+::
   Normally, a command error aborts the execution of an
   enclosing script. The relax command suppresses this behavior. It is
   useful when writing regression tests that exercise failure
   cases.
...
+echo+ [_number_]::
   'echo 1' causes each
   reposurgeon command to be echoed to standard
   output just before its output.  This can be useful in constructing regression
   tests that are easily checked by eyeball.
   +version+
...
+profile+ [ _path_ ]::
   ...
   given as argument. Profile data is dumped either when you invoke
   "profile" without argument or the program exits normally.  Profile
   data is in the format used by "go tool ppreof".
