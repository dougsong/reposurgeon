...
| B | blobs                           | Most default selection sets exclude
                                        blobs; they have to be manipulated
                                        through the commits they are attached to.
| C | commits                         |
...
| action stamp (timestamp!email) | commits or tags with that timestamp and
                                   author (or committer if no author).
				   Aliases of the author are also accepted.
...
regular expression. This is not always a subset of commits selected without the
'```a```'
...
`msgout` [ >__outfile__ ] [ ``--filter=``/__regex__/ ]::
...
shell that you then execute with the '```script```' command. Supporting this
...
Recall that each commit has a list of file operations, each an *M*
...
|==============================================================================
| M + D -> D                 | If a file is modified then deleted, the result is
