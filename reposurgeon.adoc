...
`timeoffset` _offset_ [ _timezone_ ]::
   ...
   ++[+-]++_mm_:__ss__ or ++[+-]++_hh_:__mm__:__ss__.
...
had and branch tips has correct content.  This means that if you
check out a past revision from a time when a previous branch with the
same name existed but was later deleted, you won't see that branch.
...
project and each subdirectory havong its own trunk/brances/tags
layout.
...
which that's required, wth project2 left as its own branch from top
...
While someting like this could be done with repocutter sift
commands, that would not correctly resolve Subversion copies across
projectts.  This reposurgeon procedurer handles those correctly.
...
hg::
Requires core hg, the `hg-fastimport` plugin, and (unless using reposurgeon's
